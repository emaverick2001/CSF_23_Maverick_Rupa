...
// TODO unit test for hex with chars and numbers
...
UInt256 uint256_negate(UInt256 val) {
  ...
  // loop through elements from least sig -> most sig
  for (int j = 0; j < 8; j++) {
    // get val at jth element of uint256
    val_data = val.data[j];

    // negate bits in each 32bit element in u256int
    for (int i = 0; i < 32; i++) {
      // extract bit
  ...
  if (nbits >= 32) {
    ...
    for (int i = 0; i < 8; i++) {
      if (carry != 0) {
        ...
        // add carry
        result.data[i] = result.data[i] | carry_last;
      } else {
